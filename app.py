...
# ==========================================
with st.sidebar:
    st.image("Flashpoint Finds (5).png", caption="Flashpoint Finds LLC Verification Hub", use_container_width=True)
    st.markdown("---")
...
# ==========================================
def get_historical_capsule_context(year_str):
    try:
        y = int(year_str)
    except:
        return "This volume represents a designated point of record in modern narrative history."
    
    if y >= 1938 and y < 1956:
...
with left_col:
    ...
    with tab_id:
        ...
        if st.button("Initiate Chrono-Pull Search"):
            if web_query:
                with st.spinner("Querying Comic Database Records..."):
                    prompt = f"Perform a grounded search for the comic book: '{web_query}'. Retrieve publishing details, creator names, variant configurations, key issue importance indicators, LCOG trivia, and financial diagnostics."
        ...
        if st.button("Analyze Uploaded Cover Assets"):
            if front_file:
                with st.spinner("Initiating Chrono-Scan Cover Analysis..."):
                    ...
                    img_front.save(img_bytes, format='JPEG')
                    images_to_send.append({"mime_type": "image/jpeg", "data": img_bytes.getvalue()})
                    
                    if back_file:
                        img_back = Image.open(back_file)
                        img_bytes_back = io.BytesIO()
                        img_back.save(img_bytes_back, format='JPEG')
                        images_to_send.append({"mime_type": "image/jpeg", "data": img_bytes_back.getvalue()})
        ...
        # Grid input matching existing form elements
    ...
    # ------------------
    with tab_key:
        st.subheader("🗝️ Key Collector Category Configuration")
    ...
    # ------------------
    # TAB 3: CONDITION DIAGNOSTICS GRADER
    # ------------------
    with tab_grade:
        st.subheader("🔍 Deep-Dive Deduction Calibration Matrix")
        st.caption("All parameters enforce strict Heritage Auctions structural condition ceilings. Calculated raw scores are rounded to official comic scale steps.")
        
        # Setup Deduction Option Mapping Dictionaries
        options_gloss = {
        ...
        options_corners = {
            ...
            "-0.6": ("Moderate blunting / Rounding on multiple corners", 8.0),
        ...
        # Save calculations to state dynamically
        ...
        st.session_state.comic_data["notes"] = st.text_area("Add Custom Grader Notes Segment", st.session_state.comic_data["notes"])
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; background: linear-gradient(135deg, #151515, #0a0a0a); border: 4px solid #D32F2F; border-radius: 12px; padding: 25px; color: #ffffff; width: 440px; aspect-ratio: 5/7; display: flex; flex-direction: column; justify-content: space-between; box-shadow: 0 10px 30px rgba(0,0,0,0.5); overflow:hidden;">
        <div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:start; border-bottom:2px solid #D32F2F; padding-bottom:12px; margin-bottom:15px;">
                <div>
                    <h3 style="margin:0; font-family: 'Arial Black', Gadget, sans-serif; font-size:22px; letter-spacing:1px; color:#FFC107;">FLASHPOINT FINDS</h3>
                    <span style="font-size:9px; color:#aaa; text-transform:uppercase; font-weight:bold; letter-spacing:2px;">Verified Comic Evaluation</span>
                </div>
